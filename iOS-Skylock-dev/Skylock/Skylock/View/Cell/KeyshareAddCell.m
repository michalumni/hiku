...
@implementation KeyshareAddCell
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        [_addButton setTitleColor:APP_BLUE_COLOR_TINT forState:UIControlStateNormal];
        _addButton.titleLabel.font = [UIFont fontWithName:@"HelveticaNeue-Light" size:14];
...
-(void)setConstraints
{
    ...
    NSLayoutConstraint* titleLabelRight = [NSLayoutConstraint constraintWithItem:_titleLabel attribute:NSLayoutAttributeRight
                                                                       relatedBy:NSLayoutRelationEqual toItem:_addButton attribute:NSLayoutAttributeRight multiplier:1 constant:-10];
    ...
    /////
    //button
//    NSLayoutConstraint* buttonLeft = [NSLayoutConstraint constraintWithItem:_addButton attribute:NSLayoutAttributeLeft
//                                                                      relatedBy:NSLayoutRelationEqual toItem:_titleLabel attribute:NSLayoutAttributeRight multiplier:1 constant:20];
//    [self.contentView addConstraint:buttonLeft];
//
    NSLayoutConstraint* buttonRight = [NSLayoutConstraint constraintWithItem:_addButton attribute:NSLayoutAttributeRight
                                                                        relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeRight multiplier:1 constant:-15];
    [self.contentView addConstraint:buttonRight];
    //
    NSLayoutConstraint *bottomConstraintDetail = [NSLayoutConstraint constraintWithItem:_addButton attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeNotAnAttribute multiplier:1.0 constant:30];
    [_addButton addConstraint:bottomConstraintDetail];

    NSLayoutConstraint *buttonCenterConstraintNumber = [NSLayoutConstraint constraintWithItem:_addButton attribute:NSLayoutAttributeCenterY relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeCenterY multiplier:1.0 constant:0];
    [self.contentView addConstraint:buttonCenterConstraintNumber];
}
